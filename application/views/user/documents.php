<section>
    <section class="wrapper">
        <!--overview start-->
        <div class="row">
            <div class="col-lg-12">
                <ol class="breadcrumb">
                    <li><i class="fa fa-home"></i><a href="/">Главная страница</a></li>
                    <li><i class="fa fa-laptop"></i>Список документов</li>
                </ol>
            </div>
        </div>

        <div class = "row">
            <div class = "col-lg-12">

                <table style="width: 100%">
                    <thead>
                        <th style="width: 20%">Название документа</th>
                        <th style="width: 20%">Срок хранения</th>
                        <th style="width: 20%">Состояние оплаты</th>
                        <th style="width: 20%">Дата создания</th>
                        <th style="width: 20%">Действия</th>
                    </thead>
                    <tr>
                        <td>Документ 1</td>
                        <td>30 дней</td>
                        <td>Оплачено</td>
                        <td>07.01.2016</td>
                        <td>
                            <a class="glyphicon glyphicon-pencil btn btn-danger btn-xs" style="float:right;" href=""></a>
                            <a class="glyphicon glyphicon-trash btn btn-success btn-xs" style="float:right;" href=""></a>
                            <a class="glyphicon glyphicon-shopping-cart btn btn-success btn-xs" style="float:right;" href="">
                            <a class="glyphicon glyphicon-floppy-save btn btn-success btn-xs" style="float:right;" href=""></a>
                        <td>

                    </tr>

                </table>
                <br>
                <button class = "btn btn-success" style="float: right;">Что либо</button>

            </div>
        </div>




